window.BlazorWorker = function () {
    ...
    const initWorker = function (id, onReady, onMessage, initOptions) {
        ...
        worker.onmessage = function (ev) {
            ...
            if (ev.data.startsWith('BlazorWorker.WorkerCore.SimpleInstanceService.SimpleInstanceService::InitServiceResult::')) {
                window.BlazorWorker.postMessage(1, 'BlazorWorker.WorkerCore.SimpleInstanceService.SimpleInstanceService::InitInstance::|1|3|Wasm.EvalService|Wasm, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null')
            }
            else if (ev.data.startsWith('BlazorWorker.WorkerCore.SimpleInstanceService.SimpleInstanceService::InitInstanceResult::|1|1||')) {
                onReady();
            }
            else {
                onMessage(ev.data);
